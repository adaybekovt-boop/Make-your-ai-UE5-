...
def category(obj, bindings):
    text = (obj.name + ' ' + ' '.join(bindings)).lower()
...
def export(source: Path, out: Path) -> dict:
    ...
    report = dict(schema=1, source_file=source.name, source_sha256=original_hash,
                  ...
                  coordinates='UE_X_NEGY_Z_CM', format='MAIMSH01', meshes=meshes,
    ...
    def geometry(obj, depsgraph):
        ...
        cache_key = (obj.data.as_pointer(), mirrored) if not obj.modifiers and not obj.data.shape_keys else None
        ...
        try:
            ...
            for t in mesh.loop_triangles:
                ...
                for k in order:
                    ...
                    raw.extend(CORNER.pack(index,n.x,n.y,n.z,float(tex[0]),1-float(tex[1])))
