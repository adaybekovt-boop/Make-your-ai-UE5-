...
def audit_mesh(path: Path, expected: dict | None = None) -> dict:
    ...
    for _ in range(triangles):
        ...
        for _ in range(3):
            index, nx, ny, nz, u, v, r, g, b, a = CORNER.unpack_from(raw, cursor); cursor += CORNER.size
            if index >= vertices:
                raise ValueError('Triangle vertex is outside mesh')
            finite((nx, ny, nz, u, v, r, g, b, a), 'corner'); ids.append(index)
    ...
    result = dict(vertices=vertices, triangles=triangles, material_slots=slots,
                  ...
                  degenerate_triangles=degenerate, vertex_colors=True, sha256=hashlib.sha256(raw).hexdigest(), bytes=len(raw))
    if expected:
