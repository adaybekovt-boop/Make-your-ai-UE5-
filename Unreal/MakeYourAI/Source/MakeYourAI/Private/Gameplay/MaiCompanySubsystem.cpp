...
void UMaiCompanySubsystem::Notify(const mai::Result& Result) {
    LastMessage = FText::FromString(Result.message.empty()
        ? (Result.ok ? TEXT("Action completed") : TEXT("Action failed"))
...
FMaiActionResult UMaiCompanySubsystem::NewCompany(int32 Seed) {
    ...
    ++CompanyGeneration;
    LastMessage = FText::FromString(UTF8_TO_TCHAR(mai::Loc("load.status")));
    OnChanged.Broadcast();
...
EMaiLocationStatus UMaiCompanySubsystem::GetLocationStatus(const FString& Id) const {
    return Game ? static_cast<EMaiLocationStatus>(Game->Core().LocationStatus(TCHAR_TO_UTF8(*Id))) : EMaiLocationStatus::Locked;
}
FMaiActionResult UMaiCompanySubsystem::CampaignTransact(TFunctionRef<mai::Result(mai::Campaign&)> Action) {
    ...
    auto Result = Action(*Game);
    std::string Why;
    if (Result.ok && !Game->Validate(Why)) { Game->Load(Before); Result = mai::Result::Error(Why); }
    Notify(Result); return FMaiActionResult::From(Result);
}
...
FMaiActionResult UMaiCompanySubsystem::CompleteHostLoad(const FString& Operation, bool bSuccess, const FString& Error) {
    if (!Game) return FMaiActionResult::From(mai::Result::Error("Campaign unavailable"));
    if (Game->View().loading.phase != mai::LoadPhase::Loading) {
        return FMaiActionResult::From(mai::Result::Success("No pending host load"));
    }
    const auto Generation = Game->View().loading.generation;
    Game->ReportLoading(Generation, 2500, "Resolving catalog and campaign rules");
    Game->ReportLoading(Generation, 6000, TCHAR_TO_UTF8(*Operation));
    Game->ReportLoading(Generation, 8500, "Checking SaveGame slots without blocking restore");
    auto Result = Game->CompleteLoad(Generation, bSuccess, TCHAR_TO_UTF8(*Error));
    Notify(Result); return FMaiActionResult::From(Result);
}
...
FMaiActionResult UMaiCompanySubsystem::SetPaused(bool bPaused) {
    return CampaignTransact([&](mai::Campaign& C){ C.Core().SetPaused(bPaused); return mai::Result::Success(bPaused ? "Simulation paused" : "Simulation resumed"); });
}
...
TArray<uint8> UMaiCompanySubsystem::SavePayload() const {
    TArray<uint8> Bytes;
    if (Game) { const auto Text = Game->Save(); Bytes.Append(reinterpret_cast<const uint8*>(Text.data()), static_cast<int32>(Text.size())); }
    return Bytes;
}
...
bool UMaiCompanySubsystem::Proximity(bool bInside, int64 Duration, int64 Cooldown) {
    return Game && Game->CanPlay() && Game->Core().Proximity(bInside, Duration, Cooldown);
}
