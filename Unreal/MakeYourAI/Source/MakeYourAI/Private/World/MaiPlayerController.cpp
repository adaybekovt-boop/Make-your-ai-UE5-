...
void AMaiPlayerController::BeginPlay() {
    ...
    bShowMouseCursor=true;bEnableClickEvents=true;CityCamera=Cast<AMaiCameraPawn>(GetPawn());
...
void AMaiPlayerController::PlayerTick(float DeltaTime) {
    Super::PlayerTick(DeltaTime);auto* C=GetGameInstance()->GetSubsystem<UMaiCompanySubsystem>();
...
void AMaiPlayerController::SetupInputComponent() {Super::SetupInputComponent();InputComponent->BindAction(TEXT("MaiSelect"),IE_Pressed,this,&AMaiPlayerController::ClickWorld);}
void AMaiPlayerController::ClickWorld() {
    auto* C=GetGameInstance()->GetSubsystem<UMaiCompanySubsystem>();if(!C || !C->CampaignDomain() || !C->CampaignDomain()->CanPlay() || C->CampaignDomain()->View().screen==mai::Screen::Training) return;
    FHitResult Hit;if(GetHitResultUnderCursor(ECC_Visibility,false,Hit)) {auto* A=Hit.GetActor();if(A && A->GetClass()->ImplementsInterface(UMaiInteractable::StaticClass())) IMaiInteractable::Execute_Interact(A,this);}
}
...
void AMaiPlayerController::ShowCity() {
    auto* C=GetGameInstance()->GetSubsystem<UMaiCompanySubsystem>();if(C) C->CampaignTransact([](mai::Campaign& G){return G.BeginLoad(mai::Screen::CityMap);});
}
void AMaiPlayerController::EnterLocation(const FString& Id) {
    auto* C=GetGameInstance()->GetSubsystem<UMaiCompanySubsystem>();if(C) C->CampaignTransact([&](mai::Campaign& G){return G.BeginLoad(mai::Screen::Gameplay,TCHAR_TO_UTF8(*Id));});
}
...
bool AMaiPlayerController::AtReviewDesk() const {
    const auto* Character=Cast<AMaiWalkCharacter>(GetPawn());if(!Character) return false;
...
void AMaiPlayerController::OpenTraining(bool bFromDesk) {
    ...
    auto* C=GetGameInstance()->GetSubsystem<UMaiCompanySubsystem>();if(C) C->CampaignTransact([](mai::Campaign& G){return G.ShowScreen(mai::Screen::Training);});
}
bool AMaiPlayerController::PrepareCampaignScene(const FString& Interior,bool bMenu,FString& Error) {
    ...
    if(Interior.IsEmpty()) {
        ...
        if(!bMenu) {bool Found=false;for(TActorIterator<AMaiScaffoldWorld> It(GetWorld());It;++It){Found=true;break;}if(!Found && !GetWorld()->SpawnActor<AMaiScaffoldWorld>()){Error=TEXT("City graybox spawn failed");return false;}}
